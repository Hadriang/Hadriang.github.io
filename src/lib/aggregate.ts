...
export function aggregateHardware(
  ...
  const knownPlayers = snapshot.players.filter((player) => normalizeProductName(player.gear[slot]?.name ?? "") !== "");
  ...
  const rankings = Array.from(groups.entries())
    .map(([name, players]) => ({
      ...
      percent: knownPlayers.length === 0 ? 0 : (players.length / knownPlayers.length) * 100,
  ...
  return {
    ...
    summary: {
      ...
      knownPlayers: knownPlayers.length,
      unknownPlayers: snapshot.players.length - knownPlayers.length,
